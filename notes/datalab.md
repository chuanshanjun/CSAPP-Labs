...
老外的解说:https://www.youtube.com/watch?v=0rdRRURz6IQ
Reult = 0
...
自己不熟悉的点：
    * 不能用-号，所以-1还是用+~0代替
    * x>>31 这样 如果是负数则bias 则是偏量 如果是正数则为0 这样 x + bias 可以在正数和负数的条件下都使用
...
特殊情况x=0时，!(x>>31)=0x1, !x=0x1，异或则为0
虽然x=0时，符号位与正数一样，但x=0时取反是0x1，正数取反是0x0
...
网上找了一个我比较好理解的，要求是x <= y, 分情况 1 x,y 异号且x<0，2 x,y同号,则查看x-y的符号位 3 x,y相等则直接异或
```c
int isLessOrEqual(int x, int y) {
    // 1 先取符号位
